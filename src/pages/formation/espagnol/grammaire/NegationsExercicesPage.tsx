...
import { negationsExercises, getNegationsExercisesByType, negationsExerciseCategories } from '@/data/spanishNegationsExercisesData';

const NegationsExercicesPage = () => {
  ...
  const filteredExercises = filterExercises(getNegationsExercisesByType(selectedExerciseType));
  ...
  const validateAnswer = (exerciseId: string) => {
    const exercise = negationsExercises.find(ex => ex.id === exerciseId);
    if (!exercise) return;
  ...
  const calculateScore = () => {
    ...
    negationsExercises.forEach(exercise => {
      totalQuestions++;
          ...
          <div className={`rounded-lg p-4 border-2 ${
            ...
            {!validatedAnswers[exerciseId] && (
              <p className="text-red-700 mt-2">
                La bonne réponse est : <strong>{negationsExercises.find(ex => ex.id === exerciseId)?.correctAnswer}</strong>
              </p>
            )}
          </div>
        )}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                >
                  ...
                  {negationsExerciseCategories.map(category => (
              ...
              <div className={`text-lg ${
                ...
                {score >= 80 ? (
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {examMode ? 'Félicitations ! Vous avez réussi l\'examen avec brio.' : 'Excellent ! Vous maîtrisez bien les négations espagnoles.'}
                  </div>
                ) : score >= 50 ? (
                  <div className="flex items-center justify-center gap-2">
...
export default NegationsExercicesPage;