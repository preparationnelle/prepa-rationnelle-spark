...
import { pronounsExercises, getPronounsExercisesByType, pronounsExerciseCategories } from '@/data/spanishPronounsExercisesData';

const PronounsExercicesPage = () => {
  ...
  const filteredExercises = filterExercises(getPronounsExercisesByType(selectedExerciseType));
  ...
  const validateAnswer = (exerciseId: string) => {
    const exercise = pronounsExercises.find(ex => ex.id === exerciseId);
    if (!exercise) return;
  ...
  const calculateScore = () => {
    ...
    pronounsExercises.forEach(exercise => {
      totalQuestions++;
          ...
          <div className={`rounded-lg p-4 border-2 ${
            ...
            {!validatedAnswers[exerciseId] && (
              <p className="text-red-700 mt-2">
                La bonne réponse est : <strong>{pronounsExercises.find(ex => ex.id === exerciseId)?.correctAnswer}</strong>
              </p>
            )}
          </div>
        )}
      ...
      <nav className="sticky top-0 z-40 carnet-paper-plain border-b border-dashed border-[rgba(78,55,30,0.18)]">
        <div className="container mx-auto px-4 py-2">
          <div className="flex items-center font-instrument text-[12px] text-carnet-ink-mute">
            ...
            <ChevronRight className="h-3 w-3 opacity-50 mx-2" />
            <Link to="/formation/espagnol/grammaire/pronouns" className="hover:text-carnet-red transition-colors">
              Pronoms
            </Link>
            <ChevronRight className="h-3 w-3 opacity-50 mx-2" />
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-10">
          <div className="flex justify-center mb-4">
            <Link to="/formation/espagnol/grammaire/pronouns">
          ...
          <p className="text-xl text-carnet-ink-mute max-w-3xl mx-auto">
            Maîtrisez les pronoms espagnols avec ces exercices de niveau prépa ECG : leísmo, enclise, ordre complexe...
          </p>
          <div className="flex justify-center gap-2 mt-4">
            <Badge variant="secondary">Niveau Prépa ECG</Badge>
            <Badge variant="outline">{pronounsExercises.length} exercices</Badge>
            <Badge className="bg-purple-600">Grammaire avancée</Badge>
          </div>
        </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                >
                  ...
                  {pronounsExerciseCategories.map(category => (
        ...
        {filteredExercises.length > 0 && currentExercise && (
          <div className="mb-12">
            ...
            <Card key={currentExercise.id} className="border-2 border-purple-200 bg-gradient-to-r from-purple-50 to-pink-50">
              <CardHeader>
                <CardTitle className="text-xl flex items-center gap-2 text-purple-800">
              ...
              <CardContent className="space-y-4">
                ...
                {currentExercise.type === 'qcm' || currentExercise.type === 'choix' ? (
                  ...
                  <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                    <h4 className="font-semibold text-purple-900 mb-2">✍️ Votre réponse :</h4>
                    <input
                      type="text"
                      value={userAnswers[currentExercise.id] || ''}
                      onChange={(e) => handleAnswerChange(currentExercise.id, e.target.value)}
                      className="w-full p-2 border border-purple-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 text-lg text-carnet-ink"
        ...
        {showScore && score !== null && (
          <Card className={`mt-10 border-2 ${examMode ? 'border-[rgba(193,68,58,0.25)] bg-[rgba(193,68,58,0.05)]' : 'border-purple-200 bg-purple-50'}`}>
            <CardHeader className="text-center">
              <CardTitle className={`text-3xl flex items-center justify-center gap-3 ${examMode ? 'text-carnet-red-deep' : 'text-purple-800'}`}>
            ...
            <CardContent className="text-center">
              ...
              <div className={`text-lg ${
                examMode ? 'text-carnet-red' : 'text-purple-700'
              }`}>
                {score >= 80 ? (
                  <div className="flex items-center justify-center gap-2">
                    <Trophy className="h-6 w-6 text-yellow-500" />
                    {examMode ? 'Félicitations ! Vous maîtrisez parfaitement les pronoms espagnols.' : 'Excellent ! Vous êtes un expert des pronoms espagnols.'}
                  </div>
                ) : score >= 50 ? (
                  <div className="flex items-center justify-center gap-2">
                    <CheckCircle className="h-6 w-6 text-green-500" />
                    {examMode ? 'Bon résultat ! Vous comprenez bien les règles pronominales.' : 'Bon effort ! Continuez à pratiquer les cas complexes.'}
                  </div>
                ) : (
                  <div className="flex items-center justify-center gap-2">
              ...
              {examMode && (
                <div className="mt-4 p-3 bg-purple-50 rounded-lg border border-purple-200">
                  <p className="text-purple-700 text-sm">
                    💡 En mode normal, vous pouvez voir les explications détaillées sur leísmo, enclise, et ordre des pronoms.
                  </p>
                </div>
              )}
            </CardContent>
...
export default PronounsExercicesPage;