...
import { conjugationExercises, getExercisesByType, getExercisesByCategory, exerciseCategories } from '@/data/spanishConjugationExercisesData';

const ConjugaisonExercicesPage = () => {
  ...
  const filteredExercises = filterExercises(getExercisesByType(selectedExerciseType));
  ...
  const validateAnswer = (exerciseId: string) => {
    const exercise = conjugationExercises.find(ex => ex.id === exerciseId);
    if (!exercise) return;
  ...
  const calculateScore = () => {
    ...
    // // Choix multiples
    // choixMultiplesExercises.forEach(exercise => {
    //   totalQuestions++;
    //   if (userAnswers[exercise.id] === exercise.correctAnswer) {
    //     correctAnswers++;
    //   }
    ...
    // // Mots à compléter
    // motsACompleterExercises.forEach(exercise => {
    //   totalQuestions++;
    //   if (userAnswers[exercise.id] === exercise.correctAnswer) {
    //     correctAnswers++;
    //   }
    ...
    conjugationExercises.forEach(exercise => {
      totalQuestions++;
          ...
          <div className={`rounded-lg p-4 border-2 ${
            ...
            {!validatedAnswers[exerciseId] && (
              <p className="text-red-700 mt-2">
                La bonne réponse est : <strong>{conjugationExercises.find(ex => ex.id === exerciseId)?.correctAnswer}</strong>
              </p>
            )}
          </div>
        )}
      ...
      <nav className="sticky top-0 z-40 carnet-paper-plain border-b border-dashed border-[rgba(78,55,30,0.18)]">
        <div className="container mx-auto px-4 py-2">
          <div className="flex items-center font-instrument text-[12px] text-carnet-ink-mute">
            ...
            <ChevronRight className="h-3 w-3 opacity-50 mx-2" />
            <Link to="/formation/espagnol/grammaire/conjugaison" className="hover:text-carnet-red transition-colors">
              Conjugaison
            </Link>
            <ChevronRight className="h-3 w-3 opacity-50 mx-2" />
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-10">
          <div className="flex justify-center mb-4">
            <Link to="/formation/espagnol/grammaire/conjugaison">
          ...
          <h1 className="text-4xl font-lora text-carnet-ink mb-4 flex items-center justify-center gap-3">
            ...
            Exercices de Conjugaison et Temps Verbaux
          </h1>
          <p className="text-xl text-carnet-ink-mute max-w-3xl mx-auto">
            Testez vos connaissances avec ces exercices progressifs sur la conjugaison espagnole
          </p>
          <div className="flex justify-center gap-2 mt-4">
            <Badge variant="secondary">Niveau Intermédiaire - Avancé</Badge>
            <Badge variant="outline">{conjugationExercises.length} exercices</Badge>
          ...
          <Card className="mb-8 border-4 border-carnet-red bg-[rgba(193,68,58,0.08)] shadow-lg">
            ...
            <CardContent>
              <div className="flex flex-wrap gap-4">
                ...
                >
                  QCM ({getExercisesByType('qcm').length})
                </Button>
                <Button 
                ...
                >
                  Choix multiples ({getExercisesByType('choix').length})
                </Button>
                <Button 
                ...
                >
                  Mots à compléter ({getExercisesByType('complet').length})
                </Button>
              </div>
            </CardContent>
          </Card>
              ...
              <div className={`text-lg ${
                ...
                {score >= 80 ? (
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {examMode ? 'Félicitations ! Vous avez réussi l\'examen avec brio.' : 'Excellent ! Vous maîtrisez bien la conjugaison espagnole.'}
                  </div>
                ) : score >= 50 ? (
                  <div className="flex items-center justify-center gap-2">
...
export default ConjugaisonExercicesPage; 