...
import { probabilitesFlashcards, getProbabilitesFlashcardsByCategory, getProbabilitesFlashcardsByDifficulty, shuffleProbabilitesFlashcards } from '@/data/probabilitesFlashcardsData';
import { LatexRenderer } from '@/components/LatexRenderer';

const OteriaProbabilitesFlashcardsPage = () => {
  ...
  const [filteredCards, setFilteredCards] = useState(probabilitesFlashcards);

  // Mettre à jour les cartes filtrées
  useEffect(() => {
    let filtered = probabilitesFlashcards;

    if (selectedCategory !== 'all') {
      filtered = getProbabilitesFlashcardsByCategory(selectedCategory);
    }
    ...
    if (isShuffled) {
      filtered = shuffleProbabilitesFlashcards(filtered);
    }
  ...
  const categories = Array.from(new Set(probabilitesFlashcards.map(card => card.category)));
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b border-border/40">
        <div className="container mx-auto px-4 py-2">
          <div className="flex items-center text-xs text-teal-600">
            ...
            <ChevronRight className="h-3 w-3 text-teal-400 mx-1" />
            <span className="text-teal-600 font-medium">Séance 8 - Flashcards</span>
          </div>
        </div>
      </nav>
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-teal-900 mb-4">
            Flashcards - Introduction aux Probabilités
          </h1>
          ...
          <div className="max-w-4xl mx-auto mb-6">
            <div className="bg-white border border-blue-200 rounded-lg p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <button className="w-full bg-blue-300 text-white px-4 py-3 rounded-lg cursor-not-allowed opacity-50 font-medium">
