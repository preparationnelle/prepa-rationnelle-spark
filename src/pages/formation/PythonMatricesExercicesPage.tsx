...
const PythonMatricesExercicesPage = () => {
  ...
  const qcmQuestions = [
    ...
    },
    {
      ...
      options: [
        "A) Aucune différence", 
        "B) A * B effectue une multiplication terme à terme, np.dot(A,B) le produit matriciel", 
        "C) np.dot() est plus performant", 
        "D) A * B ne fonctionne que pour les vecteurs"
      ],
    ...
    },
    {
      ...
      options: [
        "A) 2 lignes, 3 colonnes", 
        "B) 3 lignes, 2 colonnes", 
        "C) 6 éléments au total", 
        "D) Dimensions variables"
      ],
          ...
          <Card>
            ...
            <CardContent>
              <div className="mb-4">
                ...
                <p className="mb-4">On considère le programme Python partiellement complété ci-dessous :</p>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => toggleSolution('ex-nilp')}
                className="flex items-center gap-2 mb-4"
              >
                {showSolution['ex-nilp'] ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                {showSolution['ex-nilp'] ? 'Masquer' : 'Voir'} la solution
              </Button>
              {showSolution['ex-nilp'] && (
                <div className="mt-4 p-4 bg-gray-50 border rounded">
